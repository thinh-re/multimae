...
class base_cfg:
    ...
    def __init__(
        ...
        if run_type == RUN_TYPE.KAGGLE:
            ...
            self.datasets_dir_path = os.path.join(self.mount_path, 'MyDrive', 'RGBD_SOD', 'datasets') # GoogleDrive
            self.base_datasets_working_dir_path = '/content/datasets' # Colab
            self.datasets_working_dir_path = os.path.join(self.base_datasets_working_dir_path, f'v{datasets_set}') # Colab

            '''Source code'''
            self.source_code_dir = os.path.join(self.mount_path, 'MyDrive', 'RGBD_SOD', 'sources')
        ...
        self.evaluation_metrics_columns = ['Index', 'Image path', 'MAE', 'S-measure', 'F-measure (Max-F)', 'E-measure (Max-E)']
        self.detail_evaluation_metrics_columns = [
            'Index', 'Image path', 'RGB', 'Depth', 'GT', 'Prediction', 
            'MAE', 'S-measure', 'F-measure (Max-F)', 'E-measure (Max-E)'
        ]
