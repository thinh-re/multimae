...
from .utils import count_parameters, device, cpu_device, is_cuda_available

def MULTIMAE_V2(
    lr_scale: float, image_size: int, is_colab: bool = True,
    act_fn: nn.Module = nn.GELU, decoder_depth: int = 4,
    input_patch_size: int = 16, embed_dim: int = 6144, 
    dim_tokens: int = 768, decoder_main_tasks=['rgb']
):
    """MULTIMAE
    ...
    class MultiMAE(nn.Module):
        ...
        def __init__(
            self,
                ...
                output_adapters: Optional[Dict[str, ConvNeXtAdapter]],
    ...
    class args:
        ...
        ) if is_colab else './pretrained_weights/multimae-b_98_rgb+-depth-semseg_1600e_multivit-afff3f8c.pth')
    ...
    output_adapters = {
        'semseg': ConvNeXtAdapter(
            ...
            preds_per_patch=args.patch_size, 
            depth=decoder_depth, 
...
def build_model(model: nn.Module) -> str:
    file_path = os.path.join('build', 'model.pt')
    model.to(cpu_device)
    model.eval()
