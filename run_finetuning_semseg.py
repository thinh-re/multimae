...
def train_one_epoch(
    model: torch.nn.Module, criterion: _WeightedLoss, data_loader: Iterable,
    optimizer: torch.optim.Optimizer, device: torch.device, epoch: int,
    loss_scaler: NativeScalerWithGradNormCount, max_norm: float = 0, log_writer=None, start_steps=None,
...
@torch.no_grad()
def evaluate(model: nn.Module, criterion, data_loader, device, epoch, in_domains, num_classes, dataset_name,
