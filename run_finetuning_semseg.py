...
def get_args():
    ...
    parser = argparse.ArgumentParser(
        "MultiMAE semantic segmentation fine-tuning script", add_help=False
    )
    ...
    parser.add_argument(
        "--no_load_pseudo_depth", action="store_false", dest="load_pseudo_depth"
    )
    ...
    parser.add_argument(
        "--no_learnable_pos_emb", action="store_false", dest="learnable_pos_emb"
    )
    ...
    parser.add_argument(
        "--decoder_interpolate_mode",
        type=str,
        default="bilinear",
        choices=["bilinear", "nearest"],
        help="for convnext adapter",
    )
    parser.add_argument(
        "--decoder_main_tasks",
        type=str,
    ...
    # Dataset parameters
    parser.add_argument(
        "--num_classes", default=150, type=str, help="number of semantic classes"
    )
    parser.add_argument(
        "--dataset_name", default="ade20k", type=str, help="dataset name for plotting"
    )
    parser.add_argument(
        "--data_path",
        default=data_constants.ADE_TRAIN_PATH,
        type=str,
        help="dataset path",
    )
    parser.add_argument(
        "--eval_data_path",
        default=data_constants.ADE_VAL_PATH,
        type=str,
        help="dataset path for evaluation",
    )
    parser.add_argument(
        "--test_data_path", default=None, type=str, help="dataset path for testing"
    )
    parser.add_argument(
        "--max_val_images",
        default=None,
    ...
    parser.add_argument(
        "--seg_use_void_label",
        action="store_true",
        help="label border as void instead of ignore",
    )
...
def main(args):
    ...
    dataset_train = build_semseg_dataset(
        args, data_path=args.data_path, transform=train_transform
    )
    dataset_val = build_semseg_dataset(
        args,
    ...
    if args.test_data_path is not None:
        dataset_test = build_semseg_dataset(
            args, data_path=args.test_data_path, transform=val_transform
        )
    else:
        dataset_test = None
    ...
    if args.test:
        ...
        miou, a_acc, acc, loss = (
            test_stats["mean_iou"],
            test_stats["pixel_accuracy"],
            test_stats["mean_accuracy"],
            test_stats["loss"],
        )
        print(f"* mIoU {miou:.3f} aAcc {a_acc:.3f} Acc {acc:.3f} Loss {loss:.3f}")
        exit(0)
...
def evaluate(
    ...
    scores = compute_metrics_distributed(
        ...
        device=device,
        ignore_index=utils.SEG_IGNORE_INDEX,
    )
    ...
    print(
        f"* mIoU {metric_logger.mean_iou.global_avg:.3f} aAcc {metric_logger.pixel_accuracy.global_avg:.3f} "
...
def compute_metrics_distributed(
    ...
    if utils.is_main_process():
        ordered_seg_preds = [
            result for result_part in all_seg_preds for result in result_part
        ]
        ordered_seg_gts = [
            result for result_part in all_seg_gts for result in result_part
        ]
