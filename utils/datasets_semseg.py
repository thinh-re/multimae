    ...
    if train:
        transform = A.Compose(
            [
                ...
                A.ColorJitter(
                    brightness=0.4, contrast=0.4, saturation=0.2, hue=0.1, p=0.5
                ),  # Color jittering from MoCo-v3 / DINO
                A.RandomScale(
                    scale_limit=(0.1 - 1, 2.0 - 1), p=1
                ),  # This is LSJ (0.1, 2.0)
                A.PadIfNeeded(
    ...
    def __call__(self, task_dict):
        ...
        for task in task_dict:
            if task in ["depth"]:
                ...
                img = task_dict[task]
                img = F.interpolate(
                    img[None, None, :, :], scale_factor=0.25, mode="nearest"
                ).long()[0, 0]
                task_dict[task] = img
