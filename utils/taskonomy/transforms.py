...
def task_transform(file, task: str, image_size=Optional[int]):
    ...
    if task in ["rgb"]:
        transform = transforms.Compose([transform_8bit, STD_IMAGENET])
    elif task in ["normal"]:
        transform = transform_8bit
    elif task in ["mask_valid"]:
        transform = transforms.Compose([transforms.ToTensor(), make_valid_mask])
    elif task in [
        "keypoints2d",
        "keypoints3d",
        "depth_euclidean",
        ...
        transform = transforms.Compose(
            [transform_16bit_single_channel, transforms.GaussianBlur(3, sigma=1)]
        )
    ...
    if "threshold_min" in task_parameters[task]:
        ...
        transform = transforms.Compose(
            [transform, lambda x: torch.threshold(x, threshold, 0.0)]
        )
