...
class TaskonomyDataset(Dataset):
    def __init__(
        ...
        self.image_ids = pd.read_csv(
            os.path.join(
                os.path.dirname(__file__), "splits", f"{self.variant}_{self.split}.csv"
            )
        ).to_numpy()
        ...
        print(
            f"Initialized TaskonomyDataset with {len(self.image_ids)} images from variant {self.variant} in split {self.split}."
        )
    ...
    def __getitem__(self, index):
        ...
        for task in self.tasks:
            ...
            path = os.path.join(
                ...
                task,
                building,
                f"point_{point}_view_{view}_domain_{task_id}.png",
            )
            img = Image.open(path)
