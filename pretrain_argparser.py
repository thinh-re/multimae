...
class PretrainArgparser(Tap):
    ...
    save_ckpt_freq: Optional[int] = 20 # Checkpoint saving frequency in epochs (default: %(default)s)')
    ...
    num_encoded_tokens: Optional[int] = 98 # Total would be 196 * 3 patches. 196 / 2 = 98
    ...
    sample_tasks_uniformly: Optional[bool] = False # Set to True/False to enable/disable uniform sampling over tasks to sample masks for.
    decoder_use_task_queries: Optional[bool] = True
    decoder_use_xattn: Optional[bool] = True
    decoder_dim: Optional[int] = 256
    decoder_depth: Optional[int] = 2 # Number of self-attention layers after the initial cross attention (default: %(default)s)
    ...
    skip_grad: Optional[float] = None # Skip update if gradient norm larger than threshold
    ...
    balance_lr_scale: Optional[float] = 1.0
