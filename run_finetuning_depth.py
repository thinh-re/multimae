...
def get_args():
    ...
    parser = argparse.ArgumentParser(
        "MultiMAE depth fine-tuning script", add_help=False
    )
    ...
    parser.add_argument(
        "--no_scale_input_lr", action="store_false", dest="scale_input_lr"
    )
    parser.set_defaults(scale_input_lr=True)
    parser.add_argument("--freeze_transformer", action="store_true")
    parser.add_argument(
        "--no_freeze_transformer", action="store_false", dest="freeze_transformer"
    )
    ...
    parser.add_argument(
        "--data_path",
        default=data_constants.NYU_TRAIN_PATH,
        type=str,
        help="dataset path",
    )
    parser.add_argument(
        "--eval_data_path",
        default=data_constants.NYU_TEST_PATH,
        type=str,
    ...
    parser.add_argument(
        "--no_color_augs", dest="color_augs", default=False, action="store_false"
    )
    parser.add_argument(
        "--eval_freq", default=1, type=int, help="frequency of evaluation"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--max_test_images", default=100, type=int, help="number of test images"
    )
    ...
    parser.add_argument(
        "--wandb_entity", default=None, type=str, help="user or team name of wandb"
    )
    parser.add_argument(
...
def main(args):
    ...
    additional_targets = {
        domain: DOMAIN_CONF[domain]["aug_type"] for domain in args.all_domains
    }

    if args.aug_name == "nyu-augs":
        train_transform = nyu_transform(
            train=True,
            additional_targets=additional_targets,
            input_size=args.input_size,
            color_aug=args.color_augs,
        )
        val_transform = nyu_transform(
            train=False,
            additional_targets=additional_targets,
            input_size=args.input_size,
        )
    ...
    dataset_train = build_regression_dataset(
        args, data_path=args.data_path, transform=train_transform
    )
    dataset_val = build_regression_dataset(
        args,
    ...
    if args.loss == "l1":
        tasks_loss_fn = {"depth": masked_l1_loss}
    elif args.loss == "berhu":
        tasks_loss_fn = {"depth": masked_berhu_loss}
    else:
        raise NotImplementedError
    ...
    for step, x in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if standardize_depth and "depth" in input_dict:
            ...
            trunc_depth = torch.sort(
                rearrange(nan_depth, "b c h w -> b (c h w)"), dim=1
            )[0]
            ...
            robust_means = torch.stack(
                [
                    ...
                    for batch_idx, (from_idx, to_idx) in enumerate(
                        zip(from_idxs, to_idxs)
                    )
                ]
            )
            ...
            input_dict["depth"] = (
                input_dict["depth"] - robust_means[:, None, None, None]
            ) / torch.sqrt(robust_vars[:, None, None, None] + 1e-6)
        ...
        for task in input_dict:
            ...
            input_dict[task][
                ~tasks_dict["mask_valid"].repeat_interleave(repeats=channels, dim=1)
            ] = 0.0
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            task_losses = {
                task: tasks_loss_fn[task](
                    preds[task].float(),
                    tasks_dict[task],
                    mask_valid=tasks_dict["mask_valid"],
                )
                for task in preds
            }
        ...
        metrics = masked_nyu_metrics(
            preds["depth"], tasks_dict["depth"], mask_valid=tasks_dict["mask_valid"]
        )
        ...
        if log_images and pred_images is None and utils.is_main_process():
            ...
            gt_images = {
                task: v.detach().cpu().float() for task, v in input_dict.items()
            }
            gt_images.update(
                {
                    task: v.detach().cpu().float()
                    for task, v in tasks_dict.items()
                    if task not in gt_images
                }
            )
...
def evaluate(
    ...
    for x in metric_logger.log_every(data_loader, print_freq, header):
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            task_losses = {
                task: tasks_loss_fn[task](
                    preds[task], tasks_dict[task], mask_valid=tasks_dict["mask_valid"]
                )
                for task in preds
            }
            loss = sum(task_losses.values())
