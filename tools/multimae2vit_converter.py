...
def multimae_to_vit(multimae_state_dict):
    ...
    for k, v in multimae_state_dict.items():
        if k == "global_tokens":
            ...
            state_dict["pos_embed"] = F.pad(
                state_dict["pos_embed"], (0, 0, 1, 0, 0, 0), mode="constant", value=0.0
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--multimae_ckpt_path", type=str, help="Path to MultiMAE checkpoint"
    )
    parser.add_argument(
        "--vit_ckpt_path", type=str, help="Path to converted ViT(MultiMAE) checkpoint"
    )
